        ...
        <div class="col-md-12">
            <div class="card mb-4">
                <div class="d-flex justify-content-between align-items-center">
                    <h5 class="card-header">Purchase Quotations <?php echo ($quote_item) ? ' Edit' : ' Add'; ?></h5>
                ...
                <div class="card-body">
                    ...
                    <form action="<?php echo base_url() . 'admin/salesquote/insertSalesquoteItem/'. (($quote_item) ? $quote_item->quote_id : '')?>" id="add_quotaion_form" name="add_quotaion_form" method="post" autocomplete="off">
                        <div class="row">
                                                            ...
                                                            <select class="form-control dropdown-toggle quote_package_name" id="quote_package_name_<?=$i?>" name="quote_package_name[]" data-package-index=<?=$i?> required="required" style="color: #777; margin-top: 0px; font-weight: 400; height: 40px;">
                                                                ...
                                                                <?php if(!empty($packages) && count($packages)>0) :
                                                                        foreach ($packages as $package) : ?>
                                                                            <option value="<?php echo $package->id;?>" <?php echo set_select('quote_package_name', $package->id); ?> ><?php echo $package->package_name; ?></option>
                                                                <?php   endforeach; endif; ?>
                                                            </select>
                                                        </td>
                                                        <td><input type="text" class="form-control quote_package_category" id="quote_package_category_<?=$i?>" name="quote_package_category[]" required="required" readonly /></td>
                                                        <td>
                                                            <input type="text" class="form-control package_items_txt" id="package_items_txt_<?=$i?>"  name="package_items_txt[]" required="required" readonly placeholder="Package Items" aria-describedby="defaultFormControlHelp" data-bs-toggle="modal" data-bs-target="#backDropModal_<?=$i?>">                                                             
                                                        </td>
                                                        <td><input type="text" class="form-control price" id="price_<?=$i?>" name="price[]" required="required" readonly /></td>
                                                        <td><input type="date" class="form-control order_date" id="order_date_<?=$i?>" name="order_date[]" required="required"  /></td>
                                                        <td><input type="number" class="form-control quantity" id="quantity_<?=$i?>" name="quantity[]" required="required" /></td>
                                                        <td><input type="text" class="form-control final_price" id="final_price_<?=$i?>" name="final_price[]" required="required" /></td>
                                                        <!-- <td><button type="button" name="add" id="add" class="btn btn-success"><i class='bx bx-plus'></i></button></td> -->
                                                        <?php if($i!=1) {?>
                                                            <td><button type="button" name="remove" id="<?=$i?>" class="btn btn-danger btn_remove .btn-reset">Reset</button></td>
                                                        <?php } ?>
                                                    </tr> 
                                                    <?php endfor; endif;?>
                                                </tbody> 
